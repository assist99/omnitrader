...
import { MarketReversalTable } from './MarketReversalTable';
...
export default function ScreenerPage() {
  ...
  async function fetchItems(refresh = false) {
    ...
    try {
      ...
      const data = await engineFetch(`/api/screener?enabled=${enabledParam}`);
  ...
  const handleToggle = useCallback(async (id: number) => {
    const data = await engineFetch(`/api/screener/${id}/toggle`, { method: 'PATCH' });
    if (data.success) fetchItems();
  }, [tab]);

  const handleDelete = useCallback(async (id: number) => {
    if (!confirm('Delete this screener item?')) return;
    const data = await engineFetch(`/api/screener/${id}`, { method: 'DELETE' });
    if (data.success) fetchItems();
  }, [tab]);

  const getSignalColor = (signal: string | null) => {
    if (signal === 'bullish_crossover') return 'text-green-400';
    if (signal === 'bearish_crossover') return 'text-red-400';
    return 'text-slate-500';
  };

  const getSignalLabel = (signal: string | null) => {
    if (signal === 'bullish_crossover') return 'Bull';
    if (signal === 'bearish_crossover') return 'Bear';
    return '-';
  };

  const getIndicatorLabel = (type: string) => {
    if (type.toLowerCase() === 'supertrend') return 'ST';
    if (type.toLowerCase() === 'macd') return 'MACD';
    if (type.toLowerCase() === 'ema') return 'EMA';
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/dashboard/screener/new"
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-slate-700/50 bg-slate-800">
          <MarketReversalTable items={items} />
        </div>
      ) : items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-slate-500">
          <p className="text-lg">No screener items found</p>
          <Link href="/dashboard/screener/new" className="mt-2 text-sm text-blue-400 hover:text-blue-300">
            ...
            <div key={item.id} className="rounded-lg border border-slate-700/50 bg-slate-800 px-3 py-2 sm:px-4 sm:py-2 transition-colors hover:border-slate-600">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 sm:gap-3 text-sm">
                  ...
                  <span className="text-xs text-slate-400">{getIndicatorLabel(item.indicator_type)}</span>
                  ...
                  <span className="hidden sm:inline text-xs text-slate-600">|</span>
                  <span className={`hidden sm:inline text-xs font-medium ${getSignalColor(item.last_signal)}`}>
                    {getSignalLabel(item.last_signal)}
                  </span>
                </div>
                <div className="flex items-center gap-1">
                  <Link href={`/dashboard/screener/${item.id}/edit`}
